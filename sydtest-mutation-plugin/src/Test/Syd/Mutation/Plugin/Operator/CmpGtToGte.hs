...
module Test.Syd.Mutation.Plugin.Operator.CmpGtToGte (theOperator) where
...
theOperator =
  MutationOperator
    { operatorName = "CmpGtToGte",
      operatorDescription = "Replace strict greater-than (>) with greater-than-or-equal (>=)",
      operatorMatch = \case
        (L _ (OpApp _ l op r))
          | opOccName op == Just ">" ->
              Just (action l op r)
