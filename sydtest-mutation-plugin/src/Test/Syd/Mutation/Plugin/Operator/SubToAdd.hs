...
module Test.Syd.Mutation.Plugin.Operator.SubToAdd (theOperator) where
...
theOperator =
  MutationOperator
    { operatorName = "SubToAdd",
      operatorDescription = "Replace subtraction (-) with addition (+)",
      operatorMatch = \case
        (L _ (OpApp _ l op r))
          | opOccName op == Just "-" ->
              Just (action l op r)
...
action l op r = do
  ...
  let ty = fromMaybe (panic "SubToAdd: no type on left operand") (lhsExprType l)
  pure (ty, repl, "-", "+")
